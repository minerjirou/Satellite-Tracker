...
CelesTrak の実測軌道要素をもとに、いま地球を回っている約16,000基の人工衛星を
...
- 規模: TypeScript / TSX / JS / CSS 合計 **5,309 行**、コミット 5 本
- 制作日: 2026年8月20日〜21日
...
| 形式 | サイズ | 備考 |
|---|---|---|
| `GROUP=active&FORMAT=json` | 6.9 MB | OMM JSON。冗長 |
| `GROUP=active&FORMAT=3le` | **2.70 MB** | 名前行 + TLE 2行。**採用** |
...
なお `last-30-days` は `404` + 本文 `No GP data found` を返しました。グループ名は正しく、単に該当データが無い状態です。**これを障害として扱うと毎日1件の偽アラートが出る**ので、空集合として正常処理する必要がありました。
...
flowchart LR
    ...
    CRON -->|2時間に1回<br/>active 3LE 2.7MB| CT
    CRON -->|tle:active| KV
    GA -->|1日1回<br/>15グループ 2.0MB| CT
    GA -->|groups:v1| KV
    KV --> W
    W -->|/api/tle<br/>/api/groups| B
    A -->|HTML/JS/テクスチャ| B
```
...
- 頻繁で軽い TLE 本体 → Worker の Cron Trigger（バイト列を KV に流すだけ）
...
| データ | サイズ | 頻度 | 実行元 | 日次 |
|---|---|---|---|---|
| `active` 3LE | 2.70 MB | 12回/日 | Cloudflare Worker | 32.4 MB |
| グループ15種 2LE | 2.0 MB | 1回/日 | GitHub Actions | 2.0 MB |
| **合計** | | | | **約 34 MB/日** |
...
// 返すときも
const cached = await env.SATCACHE.getWithMetadata(KEY_TLE, { type: 'stream' });
return new Response(cached.value, { headers });
```

`res.text()` を避けているのは、**2.7MB の UTF-8 デコードと文字列化だけで CPU 予算を使い切る**からです。`arrayBuffer()` ならバイト列のコピーだけで済みます。

同じ理由で「衛星の件数」も Worker では数えていません。行数を数えるには全走査が必要だからです。件数はクライアントがパース時に算出して表示します。

`/api/meta` で更新時刻を返すときも、`KV.list()` が**値を読まずに metadata だけ返す**性質を使い、2.7MB の TLE を一切ロードせずに済ませています。
...
### 5.7 `last-30-days` の 404 を障害扱いしていた
...
グループ名は正しく、単に該当データが無い状態でした。障害として扱うと毎日1件の偽アラートが出るため、`404` かつ本文が `No GP data found` のときは**空集合として正常処理**するようにしました。これにより、そのグループのビットが正しくクリアされます。
...
デプロイ直後、KV が空の状態で `/api/tle` にアクセスしました。
...
逆に、**唯一の設計の穴（マスク0バグ）は「グループが全件を覆っている」という測っていない前提**から出ました。実データを画面に出すまで気づけない類のバグです。
...
**`last-30-days` が常に空** — CelesTrak 側が現在このグループに対して `No GP data found` を返しています。アプリ側は正常に空集合として処理していますが、フィルタ UI には常に0件で表示されます。

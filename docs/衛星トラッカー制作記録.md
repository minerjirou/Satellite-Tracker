...
> 気づかず、327 基を取りこぼしていました。詳細は [5.7](#57-last-30-days-が常に空--実は形式の問題だった) を参照。
...
flowchart LR
    ...
    CRON -->|2時間に1回<br/>active CSV 2.5MB| CT
    CRON -->|gp:active| KV
...
| データ | サイズ | 頻度 | 実行元 | 日次 |
|---|---|---|---|---|
| `active` CSV | 2.48 MB | 12回/日 | Cloudflare Worker | 29.8 MB |
| グループ15種 CSV | 2.0 MB | 1回/日 | GitHub Actions | 2.0 MB |
| **合計** | | | | **約 32 MB/日** |
...
`res.text()` を避けているのは、**2.5MB の UTF-8 デコードと文字列化だけで CPU 予算を使い切る**からです。`arrayBuffer()` ならバイト列のコピーだけで済みます。

同じ理由で「衛星の件数」も Worker では数えていません。行数を数えるには全走査が必要だからです。件数はクライアントがパース時に算出して表示します。

`/api/meta` で更新時刻を返すときも、`KV.list()` が**値を読まずに metadata だけ返す**性質を使い、2.5MB の本体を一切ロードせずに済ませています。
...
対処として、データ形式を全面的に CSV へ移行しました。

- Worker: `FORMAT=3le` → `FORMAT=csv`、KV キーとエンドポイントも `gp:active` / `/api/gp` に改名
- 伝播 Worker: `twoline2satrec` による桁位置パース → CSV 行をそのまま OMM として `json2satrec` へ
- グループ集計: `FORMAT=2le` → `FORMAT=csv`（Alpha-5 デコード処理は不要になり削除）
...
| データ形式 | 3LE | **CSV**（3LE では 327 基が落ちる） |
